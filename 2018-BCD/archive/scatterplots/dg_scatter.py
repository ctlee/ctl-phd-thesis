...
df = pd.read_excel('~/Desktop/BCD_for_pandas.xlsx', sheetname='dg_pandas')
#df=df.set_index(['Ligand'])
df_err=df.copy()
#df=df.drop(labels='error',axis=1)
df=df.pivot(index='Ligand', columns='Method', values='dg')
df=df.sort_values('Experiment', axis=0)
df_err['dg']=df_err['error']
df_err=df_err.drop(labels='error', axis=1)
df_err=df_err.pivot(index='Ligand', columns='Method', values='dg')
...
ttl= 'Calculated Binding Free Energies'
#df.plot( y='', kind='scatter',ax=ax) 
...
ax.set_ylabel('$\Delta$G (kcal/mol)', size= 14)
ax.set_title('Calculated Binding Free Energies', size=18)
plt.tight_layout()
#ax.margins(0.05)
#print ax.get_xbound
...
fig.savefig('dg_scatter.png', dpi=300)
#plt.show()
#print df.index
