...
df = pd.read_excel('~/Desktop/BCD_for_pandas.xlsx', sheetname='off_pandas')
#df=df.set_index(['Ligand'])
df_err=df.copy()
#df=df.drop(labels='error',axis=1)
df=df.pivot(index='Ligand', columns='Method', values='k off')
df=df.sort_values('Experiment', axis=0)
print df_err
df_err['k off']=df_err['error']
df_err=df_err.drop(labels='error', axis=1)
df_err=df_err.pivot(index='Ligand', columns='Method', values='k off')
...
for col, style, color in zip(df.columns, styles, colors):
    df[col].plot(logy=True,fmt=style,color=color, yerr=df_err[col], capsize=4, capthick=1, ms=7, ax=ax)
ax.legend()
...
ax.set_ylabel('k off ($\mathregular{s^{-1}}$)', size= 14)
ax.set_title('Calculated Off Rates', size=18)
plt.tight_layout()
#ax.margins(0.05)
#print ax.get_xbound
...
fig.savefig('off_scatter.png', dpi=1000)
#plt.show()
#print df.index
